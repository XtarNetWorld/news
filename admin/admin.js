...
$('#github-form').addEventListener('submit', async event => {
  event.preventDefault(); const form = event.currentTarget; const body = Object.fromEntries(new FormData(form)); syncPublishPath(); const newPath = body.path; const originalPath = form.dataset.originalPath || ''; const originalContent = form.dataset.originalContent || ''; const result = $('#github-result'); const externalImage = $('#hero-image-url').value.trim(); const routeChanged = originalPath && originalPath.replace(/^\/+/, '').replace(/\/index\.html$/, '/') !== newPath;
  if (originalPath && originalContent === body.content && !uploadedHero && !externalImage && !routeChanged) { result.textContent = 'No changes detected. Nothing was sent to the API.'; showNotice('No changes detected.', false); return; }
  result.textContent = 'Publishing article, image, cards data, sitemap, and feed…';
  try { let response;
    if (generatedPage && generatedHtml) { const slug = slugify(generatedPage.slug || generatedPage.title); const category = slugify(generatedPage.category || 'news'); if (uploadedHero) { generatedPage.heroImageUrl = ''; generatedPage.heroImageExtension = uploadedHero.extension; generatedHtml = await buildPageFromTemplate(generatedPage); } else if (externalImage) { if (!/^https:\/\//i.test(externalImage)) throw Error('Image URL must start with HTTPS.'); generatedPage.heroImageUrl = externalImage; generatedPage.heroImageExtension = /\.png(?:\?|$)/i.test(externalImage) ? 'png' : /\.webp(?:\?|$)/i.test(externalImage) ? 'webp' : 'jpg'; generatedPage.heroImageSource = externalImage; generatedHtml = await buildPageFromTemplate(generatedPage); } const extension = uploadedHero ? uploadedHero.extension : (generatedPage.heroImageUrl ? (generatedPage.heroImageExtension || 'jpg') : 'svg'); const assetPath = `public/${slug}-hero.${extension}`; response = await api('/publish', { method: 'POST', body: JSON.stringify({ articlePath: newPath, articleContent: generatedHtml, assetPath, assetContent: uploadedHero || generatedPage.heroImageUrl ? '' : coverSvg(generatedPage), uploadedImageBase64: uploadedHero?.base64 || '', sourceImageUrl: uploadedHero ? '' : (generatedPage.heroImageUrl || ''), article: generatedPage }) }); }
    else if (originalPath && (uploadedHero || externalImage)) { const doc = new DOMParser().parseFromString(body.content, 'text/html'); const slug = slugify($('#publish-slug').value); const extension = uploadedHero ? uploadedHero.extension : (/\.png(?:\?|$)/i.test(externalImage) ? 'png' : /\.webp(?:\?|$)/i.test(externalImage) ? 'webp' : 'jpg'); const assetPath = `public/${slug}-hero.${extension}`; const image = doc.querySelector('.post-hero img'); if (!image) throw Error('This article has no hero image element to update.'); image.src = `/${assetPath}`; body.content = `<!doctype html>\n${doc.documentElement.outerHTML}`; const meta = JSON.parse(form.dataset.articleMeta || '{}'); response = await api('/publish', { method: 'POST', body: JSON.stringify({ articlePath: newPath, articleContent: body.content, assetPath, assetContent: '', uploadedImageBase64: uploadedHero?.base64 || '', sourceImageUrl: uploadedHero ? '' : externalImage, article: { ...meta, title: meta.title || slug, slug, category: $('#publish-category').value, heroAlt: image.alt || meta.heroAlt || meta.title || slug, heroImageSource: externalImage } }) }); if (response.ok && routeChanged) { const moved = await api('/move', { method: 'POST', body: JSON.stringify({ oldPath: originalPath, newPath, content: body.content, id: originalPath.split('/')[1]?.replace(/\/index\.html$/, ''), oldUrl: `/${originalPath.replace(/^\/+/, '').replace(/\/index\.html$/, '')}/` }) }); if (!moved.ok) response = moved; } }
    else if (routeChanged) response = await api('/move', { method: 'POST', body: JSON.stringify({ oldPath: originalPath, newPath, content: body.content, id: originalPath.split('/')[1]?.replace(/\/index\.html$/, ''), oldUrl: `/${originalPath.replace(/^\/+/, '').replace(/\/index\.html$/, '')}/` }) });
    else response = await api('/github/commit', { method: 'POST', body: JSON.stringify({ path: newPath.replace(/\/$/, '') + (newPath.endsWith('/') ? 'index.html' : ''), message: body.message, content: body.content }) });
    const message = await response.text(); if (!response.ok) throw Error(`Publish failed: ${message}`); result.textContent = generatedPage ? 'Published article and SEO-renamed hero image, then updated cards, sitemap, and feed.' : (uploadedHero || externalImage) ? 'New hero image uploaded and connected to the article.' : originalPath ? 'Article updated successfully.' : 'Published successfully.'; form.dataset.originalPath = newPath; form.dataset.originalContent = body.content; uploadedHero = null;
  } catch (error) { result.textContent = error.message || 'Publish failed.'; }
});
