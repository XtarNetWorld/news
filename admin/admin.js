...
async function indexArticle(path, button) {
  ...
  try { const response = await api('/index', { method: 'POST', body: JSON.stringify({ url }) }); const data = await response.json(); if (!response.ok) throw Error(data.error || data.message || 'Google indexing request failed.'); button.textContent = 'Indexed'; showNotice(`Indexing request submitted successfully for ${url}`); } catch (error) { button.textContent = original; showNotice(error.message || 'Indexing failed.', true); } finally { button.disabled = false; }
}

async function editArticle(path) {
  showNotice('Loading article editor…');
  try { const response = await api(`/article?path=${encodeURIComponent(path)}`); const data = await response.json(); if (!response.ok) throw Error(data.error || 'Article could not be loaded.'); $('#github-form [name="path"]').value = path; $('#github-form [name="content"]').value = data.content; $('#github-form [name="message"]').value = 'Edit published article'; const parsed = new DOMParser().parseFromString(data.content, 'text/html'); const image = parsed.querySelector('.post-hero img'); const pageUrl = `https://www.newsxphere.com/${path.replace(/\/index\.html$/, '/')}`; if (image?.getAttribute('src')) { const imageUrl = new URL(image.getAttribute('src'), pageUrl).href; previousImageBox.innerHTML = `<span>Previous hero image</span><img src="${escapeHtml(imageUrl)}" alt="Previous hero image" loading="lazy">`; previousImageBox.hidden = false; } else previousImageBox.hidden = true; navigate('publish'); showNotice('Article loaded. Previous image is shown below; choose a new image if needed.'); } catch (error) { showNotice(error.message, true); }
}
...
$('#writer-form').addEventListener('submit', async event => {
  event.preventDefault(); const form = event.currentTarget; const body = Object.fromEntries(new FormData(form)); body.blocks = [...form.querySelectorAll('input[name="blocks"]:checked')].map(input => input.value); if (form.querySelector('input[name="autoBlocks"]')?.checked) body.blocks.unshift('auto-select'); body.mode = 'page';
